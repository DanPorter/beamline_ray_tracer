...
def vector_intersection3d(point1, direction1, point2, direction2):
    ...
    line = point2 - point1
    if mag(line) == 0:
        # points match - that is the intersection!
        return point1
...
def vector_shortest(point1, direction1, point2, direction2):
    """
    Calculate the shortest line between two lines in 3d.
    If lines are parallel, return point1, point2, dist(point2-point1)
    For derivation, see: http://paulbourke.net/geometry/pointlineplane/ "The shortest line between two lines in 3D"
    :param point1: [x,y,z] some coordinate on line 1
    :param direction1: [dx, dy, dz] the direction of line 1
    :param point2: [x,y,z] some coordinate on line 2
    :param direction2: [dx, dy, dz] the direction of line 2
    :return p1: [x, y, z] position on line1
    :return p2: [x, y, z] position on line2
    :return distance: float distance between p1 and p2
    """
    point1 = np.asarray(point1)
    point2 = np.asarray(point2)
    direction1 = np.asarray(direction1) / np.sqrt(np.sum(np.square(direction1)))
    ...
    if np.abs(np.dot(direction1, direction2)) > 0.999:
        #print('Vectors are parallel')
        return point1, point2, np.sqrt(np.sum(np.square(point2 - point1)))

    diff = point1 - point2
    d1343 = np.dot(diff, direction2)
    d4321 = np.dot(direction2, direction1)
    d1321 = np.dot(diff, direction1)
    denom = 1 - d4321 ** 2

    numer = d1343 * d4321 - d1321
    mua = numer / denom
    mub = (d1343 + d4321 * mua)

    pa = point1 + mua * direction1
    pb = point2 + mub * direction2
    pdif = np.sqrt(np.sum(np.square(pb - pa)))
    return pa, pb, pdif
